...
class ICs(object):
    ...
    def __call__(self):
        """
        Returns the initial conditions of the :class:`SEIRModel` the
        class relates to.

        Returns
        -------
        List of lists
            List of the initial conditions of the :class:`SEIRModel`
...
class RegParameters(object):
    """RegParameters:
    Base class for the regional and time dependent parameters of the  model:
    ...
    def __call__(self):
        ...
        :class:`SEIRModel` the class relates to.
...
class DiseaseParameters(object):
    """DiseaseParameters:
    Base class for the disease-specific parameters of the  model:
    a deterministic SEIR used by the Public Health England to model the
    Covid-19 epidemic in UK based on region.
    ...
    def __call__(self):
        """
        Returns the disease-specific parameters of the :class:`SEIRModel`
        the class relates to.
...
class SimParameters(object):
    """SimParameters:
    Base class for the simulation method's parameters of the  model:
    a deterministic SEIR used by the Public Health England to model the
    Covid-19 epidemic in UK based on region.
    ...
    def __call__(self):
        """
        Returns the simulation method's parameters of the :class:`SEIRModel`
        the class relates to.
...
class ParametersController(object):
    """ParametersController Class:
    Base class for the paramaters of the  model: a deterministic SEIR used
    by the Public Health England to model the Covid-19 epidemic in UK based on
    region.

    In order to simulate using the  model, the following parameters are
    ...
    def _check_parameters_input(
        ...
        if not isinstance(regional_parameters, RegParameters):
            raise TypeError('The model`s regional and time dependent\
                parameters must be of a  SEIR Model.')
    ...
    def __call__(self):
        ...
        Returns the list of all the parameters used for the simulation of the
         model in their order, which will be then separated within the
        :class:`SEIRModel` class.

        Returns
        -------
        list
            List of all the parameters used for the simulation of the
             model in their order.
